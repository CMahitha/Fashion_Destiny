...
import {
  ...
  export const createPayment = (reqData) => async (dispatch) => {
    ...
    try {
      ...
      const { data } = await axios.post(`http://localhost:8080/api/payments/${reqData.orderId}`,reqData, config);
  ...
  export const updatePayment = (reqData) => {
    return async (dispatch) => {
      ...
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/payments?payment_id=${reqData.paymentId}&order_id=${reqData.orderId}`,config);
